...
from modules.model_architecture.bert_crf_EC_cross import BERT_token_classification
from modules.datasets.dataset_bert_EC_cross import convert_mm_examples_to_features,MNERProcessor
...
if args.do_train:
    ...
    all_input_mask_addition = torch.tensor([f.input_mask_addition for f in train_features], dtype=torch.long)
    all_label_ids_addition = torch.tensor([f.label_ids_addition for f in train_features], dtype=torch.long)
    train_data = TensorDataset(all_input_ids, all_input_mask,all_segment_ids, all_input_ids2, all_input_mask2,all_segment_ids2, all_label_ids,all_input_mask_addition,all_label_ids_addition)
    if args.local_rank == -1:
    ...
    all_input_mask_addition = torch.tensor([f.input_mask_addition for f in dev_eval_features], dtype=torch.long)
    all_label_ids_addition = torch.tensor([f.label_ids_addition for f in dev_eval_features], dtype=torch.long)

    dev_eval_data = TensorDataset(all_input_ids, all_input_mask,all_segment_ids, all_input_ids2, all_input_mask2,all_segment_ids2, all_label_ids,all_input_mask_addition,all_label_ids_addition)
    ...
    for train_idx in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            input_ids, input_mask, segment_ids, input_ids2, input_mask2, segment_ids2, label_ids, input_mask_addition,label_ids_addition = batch
            neg_log_likelihood = model(input_ids, segment_ids, input_mask, input_ids2, input_mask2, segment_ids2, input_mask_addition,label_ids_addition,label_ids)
        ...
        label_map[0] = "PAD"
        for input_ids, input_mask, segment_ids, input_ids2, input_mask2, segment_ids2, label_ids, input_mask_addition,label_ids_addition in tqdm(
                dev_eval_dataloader,
                desc="Evaluating"):
            ...
            input_mask_addition = input_mask_addition.to(device)
            label_ids_addition = label_ids_addition.to(device)

            with torch.no_grad():
                predicted_label_seq_ids = model(input_ids, segment_ids, input_mask, input_ids2, segment_ids2, input_mask2, input_mask_addition)

            logits = predicted_label_seq_ids
            label_ids = label_ids.to('cpu').numpy()
            input_mask_addition = input_mask_addition.to('cpu').numpy()
            for i, mask in enumerate(input_mask_addition):
                temp_1 = []
                temp_2 = []
                tmp1_idx = []
                tmp2_idx = []
...
if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
    ...
    all_input_mask_addition = torch.tensor([f.input_mask_addition for f in eval_features], dtype=torch.long)
    all_label_ids_addition = torch.tensor([f.label_ids_addition for f in eval_features], dtype=torch.long)
    
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_input_ids2, all_input_mask2, all_segment_ids2, all_label_ids, all_input_mask_addition, all_label_ids_addition)
    # Run prediction for full data

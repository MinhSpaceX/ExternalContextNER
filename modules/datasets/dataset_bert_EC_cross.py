...
class SBInputFeatures(object):
    ...
    def __init__(self,input_ids,input_mask,segment_ids,input_ids2,input_mask2,segment_ids2,label_id,auxlabel_id,input_mask_addition,label_ids_addition):
...
class MNERProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        return ["O","B-Disease","I-Disease","B-Chemical","I-Chemical","X","[CLS]", "[SEP]"]
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for i, token in enumerate(tokens):
            ...
            if token == "[SEP]":
                ...
                ntokens2.extend(ntokens)
                segment_ids2.extend(segment_ids)
        if len(ntokens2) == 1:
            ntokens2.extend(ntokens)
            segment_ids2.extend(segment_ids)
        ntokens2.append("[SEP]")
        ...
        features.append(
            SBInputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids,input_ids2=input_ids, input_mask2=input_mask, segment_ids2=segment_ids,

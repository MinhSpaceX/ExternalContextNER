...
class MNERProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        return ["O","B-corporation","I-corporation","B-creative_work","I-creative_work","B-group","I-group","B-location","I-location","B-person","I-person","B-product","I-product","E","X","<s>", "</s>"]
 ...
 max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for i, token in enumerate(tokens):
            ...
            if token == "</s>":
                segment+=1
                tmp_mask_addition = len(tokenizer.convert_tokens_to_ids(ntokens+["</s>"]))
        ntokens.append("</s>")
        segment_ids.append(segment)
        label_ids.append(label_map["</s>"])
        auxlabel_ids.append(auxlabel_map["</s>"])

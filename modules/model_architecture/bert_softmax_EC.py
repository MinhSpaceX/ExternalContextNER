...
class BERT_token_classification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, segment_ids, input_mask, input_mask_addition, label_ids_addition = None, labels=None):
        ...
        if label_ids_addition is not None:
            ...
            if input_mask_addition is not None:
                active_loss = input_mask_addition.view(-1) == 1
                active_logits = logits.view(-1, self.num_labels)[active_loss]
                active_labels = label_ids_addition.view(-1)[active_loss]
                loss = loss_fct(active_logits, active_labels)
            else:
                loss = loss_fct(logits.view(-1, self.num_labels), label_ids_addition.view(-1))
            return loss
        else:
